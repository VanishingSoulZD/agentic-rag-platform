...
一个面向**工程实战与演示**的 Agentic RAG 项目：不仅覆盖 RAG 问答，还包含 Agent 工具链、可观测性、缓存、限流与可解释执行图。
...
### Trade-offs
- **低延迟 vs 低成本**：Cloud API 延迟/吞吐更优；vLLM batching 成本更低。
...
- **短期（1-2 周）**：统一 API 字段语义（`use` → `usage`）并补兼容层。
- **中期（2-4 周）**：强化 RAG 生成约束（引用、拒答、结构化输出），提升 answer accuracy。
- **中期并行**：上下文裁剪 + 会话摘要 + prompt 压缩，降低 prefill 成本与 TTFT。
- **后续（4-6 周）**：cloud API / vLLM 双路由灰度，按 SLA 进行流量调度。
...
**Q6：部署时选 cloud API 还是 vLLM batching？**  
A：若目标是低时延高吞吐，优先 cloud API；若目标是低成本，优先 vLLM batching。建议按业务时段做双路由灰度。

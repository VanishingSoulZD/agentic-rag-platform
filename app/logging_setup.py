...
class JsonLogFormatter(logging.Formatter):
    def format(self, record: logging.LogRecord) -> str:
        payload: dict[str, Any] = {
            ...
            'service': getattr(record, 'service', os.getenv('APP_NAME', 'agentic-rag-platform')),
...
def _build_rotating_file_handler(file_path: Path, level: int, formatter: logging.Formatter) -> RotatingFileHandler:
...
def configure_logging() -> None:
    ...
    formatter: logging.Formatter = JsonLogFormatter() if log_format == 'json' else TextLogFormatter()

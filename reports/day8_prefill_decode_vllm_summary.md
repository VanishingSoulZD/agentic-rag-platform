# Day 8 — Prefill / Decode 原理 + 云 API 快速体验（Mac 版）
...
- 环境变量从 `.env` 读取：`OPENAI_API_KEY` / `OPENAI_API_BASE` / `OPENAI_MODEL`
...
输出：`reports/day8_ttft_results.json`

## 3) 结果记录（模板）

> 当前仓库执行环境没有 `.env`（无可用云 API 凭据），因此无法在此环境直接产出真实模型结果。请在你的 Mac 本机执行上面的命令后，JSON 会自动写入真实测量值。

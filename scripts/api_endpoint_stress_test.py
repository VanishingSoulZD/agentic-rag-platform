...
async def guarded_check(coro, fallback_endpoint: str, method: str) -> EndpointCheck:
    try:
        ...
        return EndpointCheck(
            fallback_endpoint, method, 0, False, note=f"exception={exc!r}"
        )
...
async def check_chat(session: aiohttp.ClientSession, base_url: str) -> EndpointCheck:
    ...
    async with session.post(f"{base_url}/chat", json=payload) as r:
        ...
        return EndpointCheck(
            "/chat", "POST", r.status, ok, note=f"keys={list(data.keys())}"
        )


async def check_chat_stream(
    ...
    async with session.post(f"{base_url}/chat/stream", json=payload) as r:
        if r.status != 200:
            text = await r.text()
            return EndpointCheck(
                "/chat/stream", "POST", r.status, False, note=text[:200]
            )
...
async def check_agent_trace_chain(
    session: aiohttp.ClientSession, base_url: str
) -> list[EndpointCheck]:
    ...
    async with session.post(f"{base_url}/agent/trace", json=payload) as r:
        ...
        checks.append(
            EndpointCheck(
                ...
                r.status,
                ok,
                note=f"trace_id={data.get('trace_id')}",
            )
        )
        if not ok:
            return checks
    ...
    async with session.get(f"{base_url}/agent/trace/{trace_id}/view") as r:
        ...
        checks.append(
            EndpointCheck(
                f"/agent/trace/{trace_id}/view",
                "GET",
                r.status,
                ok,
                note=f"html_len={len(html)}",
            )
        )
...
async def run_chat_load(
    ...
    cost = (prompt_tokens_total / 1_000_000) * in_price_per_m + (
        completion_tokens_total / 1_000_000
    ) * out_price_per_m
...
async def main_async(args: argparse.Namespace) -> None:
    ...
    async with aiohttp.ClientSession(timeout=timeout) as session:
        checks: list[EndpointCheck] = []
        checks.append(
            await guarded_check(check_ping(session, args.base_url), "/ping", "GET")
        )
        agent_checks = await guarded_check(
            check_agent_trace_chain(session, args.base_url), "/agent/trace", "POST"
        )
        ...
        checks.append(
            await guarded_check(check_chat(session, args.base_url), "/chat", "POST")
        )
        checks.append(
            await guarded_check(
                check_chat_stream(session, args.base_url), "/chat/stream", "POST"
            )
        )
        checks.append(
            await guarded_check(
                check_rag_query(session, args.base_url), "/rag/query", "POST"
            )
        )

    ...
    with output_csv.open("w", encoding="utf-8", newline="") as fp:
        ...
        for week_start in sorted(buckets):
            ...
            hit_latencies = [
                ...
                for r in rows
                if int(r.get("cache_hit") or 0) == 1
            ]
            miss_latencies = [
                float(r.get("response_time_ms") or 0)
                for r in rows
                if int(r.get("cache_hit") or 0) == 0
            ]
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Generate weekly metrics report from request-level CSV logs."
    )
    parser.add_argument(
        "--input",
        default="reports/metrics_events.csv",
        help="Input request-level CSV file path",
    )
    parser.add_argument(
        "--output",
        default="reports/weekly_metrics_report.csv",
        help="Output report CSV file path",
    )
